...
public class DAOVisitRequestImpl implements DAOVisitRequest {
  ...
  /*
   * @Inject private VisitRequestFactory visitRequestFactory;
   * 
   * @Inject private DAOAddress daoAddress;
   * 
   * @Inject private DAOUser daoUser;
   */
  ...
  /*
   * private VisitRequestDTO createVisitRequest(ResultSet rs) throws SQLException { VisitRequestDTO visitRequest = null; if (rs.next()) { visitRequest =
   * this.visitRequestFactory.getVisitRequest(); visitRequest.setId(rs.getInt("visit_request_id"));
   * visitRequest.setRequestDate(rs.getDate("request_date")); visitRequest.setTimeSlot(rs.getString("time_slot"));
   * visitRequest.setAddress(daoAddress.getAddressById(rs.getInt("address"))); visitRequest.setStatus(VisitRequestStatus.values()[rs.getInt("status")]);
   * visitRequest.setChosenDateTime(rs.getDate("chosen_date_time")); visitRequest.setCancellationReason(rs.getString("cancellation_reason"));
   * visitRequest.setCustomer(this.daoUser.getUserById(rs.getInt("customer"))); } return visitRequest; }
   * 
   */
}
